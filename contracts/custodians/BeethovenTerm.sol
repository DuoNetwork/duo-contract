...
/// @author duo.network
contract BeethovenTerm is Custodian {
		...
		)
	{
		...
		bAdj = alphaInBP.add(BP_DENOMINATOR);
	}
	...
	function fetchPrice() public inState(State.Trading) returns (bool) {
		...
		if (navBInWei >= limitUpperInWei || navBInWei <= limitLowerInWei) {
	...
	function startPreReset() public inState(State.PreReset) returns (bool success) {
		if (block.number - lastPreResetBlockNo >= preResetWaitingBlocks) {
			...
			if (navBInWei >= limitUpperInWei) {
				...
				newBFromAPerA = 0;
				uint newBFromExcessBPerB = excessBAfterAInWei.mul(BP_DENOMINATOR).div(bAdj);
				...
				/*} else {
					...
					newBFromAPerA = excessAAfterBInWei.mul(BP_DENOMINATOR).div(bAdj);
				...
				newBFromAPerA = navAInWei.sub(navBInWei).mul(BP_DENOMINATOR).div(bAdj);
	...
	function getStates() public view returns (uint[29]) {
		return [
			...
			uint(resetState),
			alphaInBP,
			WEI_DENOMINATOR, // betaInWei
			periodCouponInWei, 
			0, // limitPeriodicInWei
			limitUpperInWei, 
			limitLowerInWei,
