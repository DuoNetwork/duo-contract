const BeethovenTerm = artifacts.require('../contracts/mocks/BeethovenTermMock');
...
contract('BeethovenTerm', accounts => {
	...
	const initContracts = async (alphaInBP = 0) => {
		...
		beethovenContract = await BeethovenTerm.new(
	...
	describe('constructor', () => {
		...
		it('bAdj should be set correctly', async () => {
			...
			assert.equal(bAdj.valueOf(), '20000', 'bAdj set incorrectly');
		});
	...
	describe('fetchPrice', () => {
		...
		it('should fetch price', async () => {
			...
			assert.isTrue(
				util.isEqual(util.fromWei(tx.logs[0].args.priceInWei), ethInitPrice.toString()) &&
