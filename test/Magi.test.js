	...
	describe('commit price', () => {
		...
		it('non pf address cannot call commitPrice method', async () => {
			try {
				...
				assert.equal(err.message, CST.VM_REVERT_MSG.revert, '');
			}
		});
		...
		it('should reject price from the same sender within cool down', async () => {
			try {
				...
				assert.equal(err.message, CST.VM_REVERT_MSG.revert, 'the VM is not reverted');
			}
		});
		...
		it('should reject price from first sender within cool down', async () => {
			...
			try {
				...
				assert.equal(err.message, CST.VM_REVERT_MSG.revert, 'third price is not rejected');
			}
		});
