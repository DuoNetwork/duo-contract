...
contract('Custodian', accounts => {
	...
	describe.only('downward reset', () => {
		function downwardReset(preBalanceA, preBalanceB, currentNavA, currentNavB) {
			let newBFromA = (currentNavA - currentNavB) / (BP_DENOMINATOR + CustodianInit.alphaInBP) * BP_DENOMINATOR * 350 / 582;
			let newAFromA = newBFromA * CustodianInit.alphaInBP / BP_DENOMINATOR;

			let newBalanceA = preBalanceA * ( 1 + newAFromA);
			let newBalanceB = preBalanceB * ( 1 + newBFromA);
			return [newBalanceA, newBalanceB];
		}
		...
		before(() =>
			initContracts()
				...
				.then(() =>
					custodianContract.balancesA
						.call(alice)
						.then(aliceA => (preBalanceAalice = aliceA.toNumber() / WEI_DENOMINATOR))
				)
				.then(() =>
					custodianContract.balancesB
						.call(alice)
						.then(aliceB => (preBalanceBalice = aliceB.toNumber() / WEI_DENOMINATOR))
				)
				.then(() =>
					custodianContract.balancesA
						.call(bob)
						.then(bobA => (preBalanceAbob = bobA.toNumber() / WEI_DENOMINATOR))
				)
				.then(() =>
					custodianContract.balancesB
						.call(bob)
						.then(bobB => (preBalanceBbob = bobB.toNumber() / WEI_DENOMINATOR))
				)
				.then(() => custodianContract.skipCooldown())
				.then(() => custodianContract.timestamp.call())
				.then(ts =>
					custodianContract
						.commitPrice(web3.utils.toWei('350'), ts.toNumber() - 200, {
							from: pf1
						})
						.then(() =>
							custodianContract.commitPrice(web3.utils.toWei('351'), ts.toNumber(), {
								from: pf2
							})
						)
				)
		...
		it('should in state downwardreset case 1', () => {
			return custodianContract.state.call().then(state => {
				assert.equal(state.valueOf(), STATE_DOWNWARD_RESET, 'not in state downward reset');
			});
		});
		...
		// 				custodianContract.balancesA.call(alice).then(currentBalanceAalice =>
		// 					custodianContract.balancesB.call(alice).then(currentBalanceBalice => {
		// 						console.log(preBalanceAalice);
		// 						console.log(currentNavA);
		// 						console.log(currentNavB);

		// 						let newBalances = downwardReset(
		// 							preBalanceAalice,
		// 							preBalanceBalice,
		// 							currentNavA, 
		// 							currentNavB
		...
		// 			.then(ts =>
		// 				custodianContract
		// 					.commitPrice(web3.utils.toWei('150'), ts.toNumber() - 200, {
		// 						from: pf1
		// 					})
		// 					.then(() =>
		// 						custodianContract.commitPrice(
		...
		// 				beta = web3.utils.fromWei(betaInWei.valueOf());
		// 				bAdj = (CustodianInit.alphaInBP + BP_DENOMINATOR) / BP_DENOMINATOR / beta;
		// 				newBFromAPerA = (currentNavA - currentNavB) / bAdj;
		// 				aAdj = CustodianInit.alphaInBP / BP_DENOMINATOR;
		// 			})
		// 			.then(() => {
		// 				let promise = Promise.resolve();
		...
		// 		);
				...
				// .then(ts =>
				// 	custodianContract
				// 		.commitPrice(web3.utils.toWei('50'), ts.toNumber() - 200, {
				// 			from: pf1
				// 		})
				// 		.then(() =>
				// 			custodianContract.commitPrice(
		...
		// it('should in state downwardreset case 3', () => {
		// 	return custodianContract.state.call().then(state => {
		// 		// assert.equal(state.valueOf(), STATE_DOWNWARD_RESET, 'not in state downward reset');
		// 		console.log(state);
		// 	});
