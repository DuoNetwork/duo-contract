	...
	describe.only('commit price', () => {
		...
		it('should accept first price arrived if it is not too far away', () => {
			return custodianContract
				.skipCooldown()
				.then(() => {
					//return custodianContract.timestamp.call().then(ts => {
					//	console.log(ts);
					return custodianContract
						.commitPrice(web3.utils.toWei('580'), initTimeStamp.toNumber() + 3100, {
							from: pf1
						})
						.then(() => assert.isTrue(true));
				})
				.then(() => custodianContract.navAInWei.call().then(navA => console.log(navA)));
		});
